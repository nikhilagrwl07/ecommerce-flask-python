...
#START CART MODULE
#Gets products in the cart-Akanksha
...
#Removes products from cart when user clicks remove-Akanksha
...
#flask form for checkout details-Akanksha
...
#Gets form data for the sales transaction-Akanksha
...
# adds data to orderdproduct table-Akanksha
...
#removes all sold products from cart for the user-Akanksha
def removeordprodfromcart(userId):
    userid= userId
    db.session.query(Cart).filter(Cart.userid == userid).delete()
    db.session.commit()

#adds sales transaction-Akanksha
...
#sends email for order confirmation-Akanksha
